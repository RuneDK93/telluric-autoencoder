...
train_size = 50
full_size = 838
selection = np.random.choice(a=np.arange(full_size),size=train_size,replace=False)
preproc_flux = preproc_flux[selection]
Airmass = Airmass[selection]
Berv = Berv[selection]
...
for order in range(0,preproc_flux.shape[1]):
    ...
    # ------------------------
    learning_rate = learning_rate / (full_size/train_size)
    # ------------------------
